...
- (void)finishPhase0;
{
    if (debug) NSLog(@"[%@] %s, changing struct names that start with $", self.identifier, sel_getName(_cmd));
    ...
    if ([_debugNames count] > 0) {
        NSLog(@"======================================================================");
        NSLog(@"[%@] %s", self.identifier, sel_getName(_cmd));
...
{
    //NSLog(@"[%@] %s, depth: %u", identifier, sel_getName(_cmd), depth);
...
- (void)phase2ReplacementOnPhase0;
{
    ...
    if (debug) NSLog(@"[%@] <  %s", self.identifier, sel_getName(_cmd));
}
                   ...
                   usedInMethod:(BOOL)isUsedInMethod
{
    ...
    if ([@"?" isEqualToString:name]) {
        ...
        if (info != nil) {
            if (debugAnonStructures) NSLog(@"%s, anon key %@ has exception from phase 2", sel_getName(_cmd), structure.typeString);
        ...
        if ([_debugNames containsObject:name]) NSLog(@"[%@] %s, type= %@", self.identifier, sel_getName(_cmd), structure.typeString);
        //NSLog(@"[%@] %s, name: %@", identifier, sel_getName(_cmd), name);
        if ([_phase3_exceptionalNames containsObject:name]) {
            if (debugNamedStructures) NSLog(@"%s, name %@ has exception from phase 2", sel_getName(_cmd), name);
            ...
            if (info == nil) {
                if ([_debugNames containsObject:name]) NSLog(@"[%@] %s, info was nil for %@", self.identifier, sel_getName(_cmd), name);
                info = [[CDStructureInfo alloc] initWithType:structure];
                [info setReferenceCount:referenceCount];
                if (isUsedInMethod)
                    info.isUsedInMethod = YES;
                ...
                if ([_debugNames containsObject:name]) NSLog(@"[%@] %s, info before: %@", self.identifier, sel_getName(_cmd), [info shortDescription]);
...
- (BOOL)shouldExpandType:(CDType *)type;
{
    ...
    if ([@"?" isEqualToString:name]) {
        ...
        if (info == nil) {
            ...
            if (info != nil) {
                //NSLog(@"[%@] %s, found phase3 name exception... %@", identifier, sel_getName(_cmd), [info shortDescription]);
                //return NO;
            }
        }
    }
...
#if 0
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, named:", identifier, sel_getName(_cmd));
...
#if 0
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, anon:", identifier, sel_getName(_cmd));
...
#if 1
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, named exceptions:", self.identifier, sel_getName(_cmd));
...
#if 0
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, anon exceptions:", identifier, sel_getName(_cmd));
...
#endif
    NSLog(@"======================================================================");
    NSLog(@"[%@] %s, anon exceptions:", self.identifier, sel_getName(_cmd));
    for (CDStructureInfo *info in [[_phase3_anonExceptions allValues] sortedArrayUsingSelector:@selector(ascendingCompareByStructureDepth:)]) {
        NSLog(@"%@", [info shortDescription]);
    }
