...
- (BOOL)canMergeWithType:(CDType *)otherType;
{
    ...
    if (_primitiveType != otherType.primitiveType) {
        ...
        if (debugMerge) NSLog(@"%s, Can't merge because of type... %@ vs %@", sel_getName(_cmd), self.typeString, otherType.typeString);
        return NO;
    }

    if (_subtype != nil && [_subtype canMergeWithType:otherType.subtype] == NO) {
        if (debugMerge) NSLog(@"%s, Can't merge subtype", sel_getName(_cmd));
        return NO;
    }

    if (_subtype == nil && otherType.subtype != nil) {
        if (debugMerge) NSLog(@"%s, This subtype is nil, other isn't.", sel_getName(_cmd));
        return NO;
    }
    ...
    //NSLog(@"%s, count: %u, otherCount: %u", sel_getName(_cmd), count, otherCount);

    if (count != 0 && otherCount == 0) {
        if (debugMerge) NSLog(@"%s, count != 0 && otherCount is 0", sel_getName(_cmd));
        return NO;
    }

    if (count != 0 && count != otherCount) {
        if (debugMerge) NSLog(@"%s, count != 0 && count != otherCount", sel_getName(_cmd));
        return NO;
    }
...
- (void)_recursivelyMergeWithType:(CDType *)otherType;
{
    ...
    if (_primitiveType != otherType.primitiveType) {
        NSLog(@"Warning: Trying to merge different types in %s", sel_getName(_cmd));
        return;
    }
...
- (void)phase0RecursivelyFixStructureNames:(BOOL)flag;
{
    ...
    if ([_typeName.name hasPrefix:@"$"]) {
        if (flag) NSLog(@"%s, changing type name %@ to ?", sel_getName(_cmd), _typeName.name);
        _typeName.name = @"?";
    }
...
- (void)phase2MergeWithTypeController:(CDTypeController *)typeController debug:(BOOL)phase2Debug;
{
    ...
    if (phase2Debug && [before isEqualToString:after] == NO) {
        NSLog(@"----------------------------------------");
        NSLog(@"%s, merge changed type", sel_getName(_cmd));
        NSLog(@"before: %@", before);
        NSLog(@" after: %@", after);
    }
}
...
{
    //NSLog(@" > %s %@", sel_getName(_cmd), [self typeString]);
    for (CDType *member in _members) {
